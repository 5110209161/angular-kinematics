...
{
  /// <summary>
  /// Session class to read and write data
  /// </summary>
  public class TcpSession : IDisposable
  {
    ...
    public virtual bool SendAsync(byte[] buffer, long offset, long size)
    {
      ...
      lock (_sendLock)
      {
        ...
        if (((_sendBufferMain.Size + size) > SendBufferLimit) && (SendBufferLimit > 0))
        {
    ...
    /// <summary>
    /// Function is called whenever a receive or send operation is completed on a socket
    /// </summary>
    ...
    /// <summary>
    /// The method is invoked when an asynchronous receive operation completes
    /// </summary>
    /// <param name="e"></param>
    /// <returns></returns>
    private bool ProcessReceive(SocketAsyncEventArgs e)
    {
      ...
      if (size > 0)
      {
        ...
        if (_receiveBuffer.Capacity == size)
        {
          ...
          if ((2 * size) > ReceiveBufferLimit && ReceiveBufferLimit > 0)
          {
            SendError(SocketError.NoBufferSpaceAvailable);
            Disconnect();
            return false;
    ...
    /// <summary>
    /// Function is called when an asynchronous send operation complete
    /// </summary>
    /// <param name="e"></param>
    /// <returns></returns>
    ...
    /// <summary>
    /// Handler client connected notification
    /// </summary>
